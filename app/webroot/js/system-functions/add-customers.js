$(document).ready(function() {
			...
			var neighborhood_value_default = $('#ClienteNeighborhood').html();
			...
			//Toda vez que mexe o campo país
			$("#ClienteCountry").change(function () {
				//Salva o valor(id) do país selecionado
				//Atualiza o campo ESTADO
				var country_selected_value = $('#ClienteCountry').val();
				//Se só existir um optgroup ele entra no if
				if($('#ClienteState optgroup').length != 3){
					//remove todos os optgroups existentes
					$('#ClienteState optgroup').remove();
					//adiciona o valor default dos optgroups no select
					$('#ClienteState').html(state_value_default);
					//se o valor do optgroup for diferente do valor do país, deleta ele mané!
					$('#ClienteState optgroup').each(function(index) {
						if($(this).attr('label') != country_selected_value){
							$(this).remove();
						}
					});
					//Atualiza também o campo CIDADE
					var state_selected_value = $('#ClienteState').val();
					//Se só existir um optgroup ele entra no if
					if($('#ClienteCity optgroup').length != 3){
						//remove todos os optgroups existentes
						$('#ClienteCity optgroup').remove();
						//adiciona o valor default dos optgroups no select
						$('#ClienteCity').html(city_value_default);
						//se o valor do optgroup for diferente do valor do país, deleta ele mané!
						$('#ClienteCity optgroup').each(function(index) {
							if($(this).attr('label') != state_selected_value){
								$(this).remove();
							}
						});
					}
					//Atualiza também o campo BAIRRO
					var city_selected_value = $('#ClienteCity').val();
					//Se só existir um optgroup ele entra no if
						if($('#ClienteNeighborhood optgroup').length != 3){
							//remove todos os optgroups existentes
							$('#ClienteNeighborhood optgroup').remove();
							//adiciona o valor default dos optgroups no select
							$('#ClienteNeighborhood').html(neighborhood_value_default);
							//se o valor do optgroup for diferente do valor do país, deleta ele mané!
							$('#ClienteNeighborhood optgroup').each(function(index) {
								if($(this).attr('label') != city_selected_value){
									$(this).remove();
								}
							});
						}
			...
			//Toda vez que mexe o campo estado
			...
			//Toda vez que mexe o campo cidade
			$("#ClienteCity").change(function () {
				//Salva o valor(id) do país selecionado
				var city_selected_value = $('#ClienteCity').val();
				//Se só existir um optgroup ele entra no if
				if($('#ClienteNeighborhood optgroup').length != 3){
			...
			//deleta os optiongroups que não são do pais selecionado
			$('#ClienteState optgroup').each(function(index) {
					//Salva o valor(id) do país selecionado
					var country_selected_value = $('#ClienteCountry').val();
			...
			//deleta os optiongroups que não são do estado selecionado
			$('#ClienteCity optgroup').each(function(index) {
					//Salva o valor(id) do país selecionado
					var state_selected_value = $('#ClienteState').val();
			...
			//deleta os optiongroups que não são da cidade selecionada
			$('#ClienteNeighborhood optgroup').each(function(index) {
					//Salva o valor(id) do país selecionado
					var city_selected_value = $('#ClienteCity').val();
